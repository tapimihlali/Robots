...
def tdi(series, rsi_lookback=13, rsi_smooth_len=2,
    ...
    rsi_smooth = sma(rsi_data, rsi_smooth_len)[20:]
    rsi_signal = sma(rsi_data, rsi_signal_len)[20:]

    bb_series = bollinger_bands(rsi_data, bb_lookback, bb_std)[20:]
    H50_line = len(rsi_data)*[50.0]
    H70_line = len(rsi_data)*[70.0]
    H30_line = len(rsi_data)*[30.0]

    return pd.DataFrame(index=series.index, data={
        "rsi_data": rsi_data[20:],
...
def Momentum(df, period=14):
    ...
    data = pd.DataFrame()
    data['Momentum'] = df['close'].diff(periods=period)

    return data

# ---------------------------------------------

def calculate_dms(df, period=14):
    
    data = pd.DataFrame()
    
    # Calculate True Range (TR)
    data['High-Low'] = df['high'] - df['low']
    data['High-Close'] = abs(df['high'] - df['close'].shift(1))
    data['Low-Close'] = abs(df['low'] - df['close'].shift(1))
    ...
    data['High-PrevHigh'] = df['high'] - df['high'].shift(1)
    data['PrevLow-Low'] = df['low'].shift(1) - df['low']
    data['+DM'] = (data['High-PrevHigh'] > data['PrevLow-Low']).astype(int) * data['High-PrevHigh']
    data['-DM'] = (data['PrevLow-Low'] > data['High-PrevHigh']).astype(int) * data['PrevLow-Low']

    # Calculate True Positive Directional Movement (+DM) and True Negative Directional Movement (-DM)
    data['True +DM'] = data['+DM'].where(data['+DM'] > data['-DM'], 0)
    data['True -DM'] = data['-DM'].where(data['-DM'] > data['+DM'], 0)

    # Calculate 14-period average of True +DM, True -DM, and TR
    data['Avg True +DM'] = data['True +DM'].rolling(window=period).mean()
    data['Avg True -DM'] = data['True -DM'].rolling(window=period).mean()
    data['Avg TR'] = data['TR'].rolling(window=period).mean()

    # Calculate Directional Indexes (+DI and -DI)
    data['+DI'] = (data['Avg True +DM'] / data['Avg TR']) * 100
    data['-DI'] = (data['Avg True -DM'] / data['Avg TR']) * 100
...
def calculate_imi(df, period=14):
    ...
    data['Typical Price'] = (df['high'] + df['low'] + df['close']) / 3

    # Calculate price changes
    data['Price Change'] = data['Typical Price'].diff()

    # Calculate direction (1 for up, -1 for down, 0 for no change)
    data['Direction'] = np.where(data['Price Change'] > 0, 1, np.where(data['Price Change'] < 0, -1, 0))

    # Calculate positive and negative price changes
    data['Positive Price Change'] = np.where(data['Direction'] == 1, data['Price Change'], 0)
    data['Negative Price Change'] = np.where(data['Direction'] == -1, abs(data['Price Change']), 0)

    # Calculate 14-period sums of positive and negative price changes
    data['Sum of Positive Price Change'] = data['Positive Price Change'].rolling(window=period).sum()
    data['Sum of Negative Price Change'] = data['Negative Price Change'].rolling(window=period).sum()

    # Calculate the IMI
    data['IMI'] = data['Sum of Positive Price Change'] / (data['Sum of Positive Price Change'] + data['Sum of Negative Price Change']) * 100
...
def chop(df, chop_date=False, chop_length=14, atr_length = 1):
    chop_df = pd.DataFrame()
    if chop_date==True:
        chop_df['date'] = df.date
    elif chop_date==False:
        chop_df['date'] = df.index
        df = df.reset_index()
    #  Calculate Choppiness
    chop_df['chop'] = ta.chop(high=df['high'], low=df['low'], close=df['close'], length=chop_length, atr_length=atr_length)
    #  Add conditions
    choppy_threshold = 61.8
    trending_threshold = 38.2
    chop_df['is_choppy'] = np.where((chop_df['chop'] > choppy_threshold), 1, 0)
    chop_df['is_trending'] = np.where((chop_df['chop'] < trending_threshold), 1, 0)
...
def volprofile(bars):
    ...
    bucket_size = 0.0002 * max(bars['close'])
    volprofile  = bars['volume'].groupby(bars['close'].apply(lambda x: bucket_size*round(x/bucket_size,0))).sum()
...
def calculate_ichimoku(df, tenkan_period=9, kijun_period=26, senkou_b_period=52):
    ...
    data['Senkou Span B'] = ((senkou_b_high + senkou_b_low) / 2).shift(kijun_period)

    return data

# ---------------------------------------------

def macd(series, fast=9, slow=25, smooth=30):
    """
...
def compute_cci(df, n=20):
    ...
    SMA_TP = TP.rolling(window=n).mean()
        ...
        # Manual calculation of Mean Absolute Deviation (MAD)
    def mad(x):
        return np.mean(np.abs(x - np.mean(x)))
    
    MD = TP.rolling(n).apply(mad, raw=False)
